...
<body>
	<!-- [데이터 공유 저장소]
	ServletContext application: app동작시 생성 ~ app 종료시 소멸, 전역공간, 모든 곳에서 접근 가능
	HttpSession session: 브라우저가 서버에 session이 형성되면 생성되는 공간, 타임아웃 시간이 넘어가면 소멸, 로그인 정보
	...
	혀재는 request에 member객체가 저장되어 있으므로
	아래의 EL은 member를 바로 꺼내 사용한다.
	
	${member.no}에서 no를 '프로퍼티'라고 부른다.
	이것은 member.getNo() 함수를 호출하는데 get은 제거하고 Ndㄴ 소문자로 바꾸는 규칙이다.
	결국, ${member.no}는 member.getNo()와 같다.
	
	만약 request공간과 session공간에 동일한 nameList 객체가 존재할 때
