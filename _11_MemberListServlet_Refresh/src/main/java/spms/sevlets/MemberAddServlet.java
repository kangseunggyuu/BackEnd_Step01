...
public class MemberAddServlet extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		// get요청은 소스에 하지 않고, tomcat의 server.xml에 해야 함.
		...
		/* statement
		 * - 질의할 떄마다 sql을 컴파일한다.
		 * - 입력 매개변수가 여러 개 필요할 때 문자열 결합 연산자인 +를 이용해서 이용해야한다.
		 * - 전송 직전에 sql문을 입력 받고, 컴파일 후, 서버로 전송
		 * 
		 *  preparestatement
		 *  - sql문을 미리 입력하여 컴파일 한상태에서 객체를 받는다.
		 *  - 만약, sql문 구조가 변경되지 않고, 파라미터 값만 바뀌는 경우 statement보다 빠르다.
		...
		try {
			...
			out.println("<meta http-equiv='Refresh' content='0.5;url=list'>");
			out.println("<body>");
